// wavIO based on code by Evan X. Merz from www.thisisnotalabel.com
...
*/
	...
	public byte[] read()
	{
		...
		try
		{
			...
			// Check if dataChunkID is actually "data"; if not (metadata present) waits until we are in the actual data chunk			
			if (false == dataChunkID.equals("data")) {
				...
				dataChunkID = (""+(char)inFile.readByte()) + dataChunkID;
				dataChunkID = dataChunkID.substring(1);
				Log.i("wavIO", dataChunkID);
				}
			}
		...
		return myData; // this should probably be something more descriptive
	}
	...
	public boolean save()
	{
		try
		{
			final DataOutputStream outFile  = new DataOutputStream(new FileOutputStream(myPath));
	...
	public String getSummary(String filename, String convertedSize)
	{
		String timeLabel;
		if (getTimeInSec() == 1) timeLabel = " second";
		else timeLabel = " seconds";
		
		String newline = "<br>";
		String summary = "<html>Name: " + filename + newline + "Duration: " + getTimeInSec() + timeLabel
				+ newline + "Format: " + strFormat + newline + "Channels: " + myChannels
				+ newline + "SampleRate: " + mySampleRate + " Hz" + newline + "ByteRate: " + myByteRate + " bytes/second"
				+ newline + "BlockAlign: " + myBlockAlign + " bytes/sample" 
				+ newline + "Quantization: " + myBitsPerSample  + " bits/sample"
				+ newline + "DataSize: " + convertedSize + "</html>";
		return summary;
	}
	...
	public int getTimeInSec(){
		return (int)(myDataSize/myByteRate);
	}
